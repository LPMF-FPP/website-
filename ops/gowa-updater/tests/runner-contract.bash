...
env PATH="$bin:/usr/bin:/bin" GOWA_UPDATER_CAPABILITY_MANIFEST="$tmp/capability.json" GOWA_UPDATER_ENABLED_MARKER="$tmp/enabled" GOWA_UPDATER_PREFLIGHT_GATE="$tmp/preflight.pass" GOWA_UPDATER_AUTHORITY_PATH="$tmp/authority.json" GOWA_UPDATER_REQUEST_ROOT="$tmp/requests" GOWA_TEST_SYSTEMCTL_LOG="$tmp/systemctl.log" bash "$(dirname "$runner")/lpmf-gowa-submit" 00000000-0000-4000-8000-000000000000
grep -F -- 'lpmf-gowa-update@00000000-0000-4000-8000-000000000000.service' "$tmp/systemctl.log" >/dev/null

env PATH="$bin:/usr/bin:/bin" GOWA_UPDATER_ENABLED=1 GOWA_UPDATER_NO_SOCKET_GATE=1 GOWA_UPDATER_CAPABILITY_MANIFEST="$tmp/capability.json" GOWA_UPDATER_CATALOG="$tmp/catalog.json" GOWA_UPDATER_CATALOG_PUBLIC_KEY="$tmp/catalog.pub" GOWA_UPDATER_ENVELOPE="$tmp/envelope.json" GOWA_UPDATER_REQUEST_ROOT="$tmp/requests" GOWA_UPDATER_EVIDENCE_ROOT="$tmp/evidence" GOWA_UPDATER_AUTHORITY_PATH="$tmp/authority.json" GOWA_UPDATER_LOCK_PATH="$tmp/run/update.lock" GOWA_UPDATER_PREFLIGHT_GATE="$tmp/preflight.pass" GOWA_UPDATER_ENABLED_MARKER="$tmp/enabled" GOWA_UPDATER_EVIDENCE_SIGNING_KEY="$tmp/evidence.key" GOWA_UPDATER_ROLLBACK_MANIFEST="$tmp/rollback.json" GOWA_UPDATER_DOCKER_BIN="$bin/docker" GOWA_UPDATER_PSQL_BIN="$bin/psql" bash "$runner" 00000000-0000-4000-8000-000000000000
test -s "$tmp/evidence/00000000-0000-4000-8000-000000000000/1/1-mutation_prepared.json"
