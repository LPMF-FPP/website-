...
jq -n --arg image "$rollback_image" --arg project "$project" --arg work "$root" --arg compose "$compose_file" --arg override "$override_file" --arg generation 'staging-real-generation-1' --argjson mounts "$mounts" '{schema_version:1,previous_image:$image,project:$project,service:"whatsapp_go",working_directory:$work,compose_files:[$compose],image_override:$override,network_mode:"host",restart_policy:"unless-stopped",mounts:$mounts,fixed_config_hash:("0"*64),catalog_generation:$generation,policy_version:"1",secret_source:"runner-owned"}' > "$root/rollback.json"
...
env PATH="$bin_dir:/usr/bin:/bin" GOWA_STAGING_PSQL_LOG="$root/psql.log" GOWA_STAGING_SYSTEMCTL_LOG="$root/systemctl.log" GOWA_UPDATER_ENABLED=1 GOWA_UPDATER_NO_SOCKET_GATE=1 GOWA_UPDATER_CAPABILITY_MANIFEST="$root/capability.json" GOWA_UPDATER_REQUEST_ROOT="$request_root" GOWA_UPDATER_DATABASE='staging' GOWA_UPDATER_SUBMIT_ROLE='lpmf_gowa_submit' bash "$submit_helper" "$operation_id"
...
for evidence_file in "${evidence[@]}"; do
    php -r 'require $argv[1]; $importer = new App\Services\WhatsApp\GowaEvidenceImporter($argv[3]); $importer->decode($argv[2]);' "$script_dir/../../../vendor/autoload.php" "$evidence_file" "$public_key"
done
