...
class NumberingRepairService
{
    ...
    public function compactRequestNumbersForBucket(CarbonImmutable $bucketNow, string $reason = 'Auto compact requests after deletions'): array
    {
        ...
        return DB::transaction(function () use ($plan, $built, $reason, $requests) {
            ...
            $counterAfter = (int) $built['ba_counter_after'];
            $this->setSequenceValue('ba', $requests->first(), $counterAfter);
            $this->setSequenceValue('tracking', $requests->first(), (int) $built['tracking_counter_after']);
