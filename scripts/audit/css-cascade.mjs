...
const CSS_PATHS = ["resources/css", "public/css", "styles"];
...
const REDUCED_MOTION_IMPORTANT_PROPS = new Set([
    "animation-duration",
...
const PRINT_IMPORTANT_PROPS = new Set([
    "background",
    "color",
    "box-shadow",
    "text-shadow",
...
function extractRightMostSelectorSegment(selector) {
    let squareDepth = 0;
    let roundDepth = 0;
    let splitIndex = -1;
    ...
    const segment = selector.slice(splitIndex + 1).trim();
    return segment || selector.trim();
}

function parseSelectorSignature(selector) {
    const signature = {
        types: [],
        classes: [],
        ids: [],
    ...
    try {
        ...
        walk(ast, {
            ...
            enter(node) {
                if (typeof node.name === "string" && node.name.trim() !== "") {
                    types.push(node.name.trim().toLowerCase());
                }
            },
        });
        ...
        walk(ast, {
            ...
            enter(node) {
                if (typeof node.name === "string" && node.name.trim() !== "") {
                    classes.push(node.name.trim().toLowerCase());
                }
            },
        });
        ...
        walk(ast, {
            ...
            enter(node) {
                if (typeof node.name === "string" && node.name.trim() !== "") {
                    ids.push(node.name.trim().toLowerCase());
                }
            },
        });
        ...
        walk(ast, {
            ...
            enter(node) {
                ...
                if (typeof raw === "string" && raw.trim() !== "") {
                    attrs.push(raw.trim().toLowerCase());
                    return;
                }
                ...
                if (
                    ...
                    attrs.push(raw.name.trim().toLowerCase());
                }
            },
        });
        ...
        walk(ast, {
            ...
            enter(node) {
                if (typeof node.name === "string" && node.name.trim() !== "") {
                    pseudoElements.push(node.name.trim().toLowerCase());
                }
            },
        });

        signature.types = uniqueSorted(types);
        signature.classes = uniqueSorted(classes);
        signature.ids = uniqueSorted(ids);
        signature.attrs = uniqueSorted(attrs);
        signature.pseudoElements = uniqueSorted(pseudoElements);
        signature.universal = universal;

        return signature;
    } catch (error) {
        const lower = selector.toLowerCase();
        signature.classes = uniqueSorted(
            Array.from(lower.matchAll(/\.([_a-z][-_a-z0-9]*)/g), (m) => m[1]),
        );
        signature.ids = uniqueSorted(
            Array.from(lower.matchAll(/#([_a-z][-_a-z0-9]*)/g), (m) => m[1]),
        );
        signature.attrs = uniqueSorted(
            Array.from(
                lower.matchAll(/\[\s*([a-z_:-][a-z0-9_:-]*)/g),
                (m) => m[1],
            ),
        );
        signature.types = uniqueSorted(
            Array.from(
                lower.matchAll(/(^|[\s>+~])([a-z][a-z0-9-]*)/g),
                (m) => m[2],
            ),
        );
        signature.pseudoElements = uniqueSorted(
            Array.from(lower.matchAll(/::([_a-z][-_a-z0-9]*)/g), (m) => m[1]),
        );
        signature.universal = lower.includes("*");

        return signature;
    }
}

function parseTerminalSelectorSignature(selector) {
    const segment = extractRightMostSelectorSegment(selector);
    return parseSelectorSignature(segment);
}
...
function selectorsMayOverlap(leftSig, rightSig) {
    if (!leftSig || !rightSig) {
        return true;
    }

    if (
        signaturesHaveSpecificTokens(leftSig) &&
        signaturesHaveSpecificTokens(rightSig) &&
        !signaturesShareSpecificTokens(leftSig, rightSig)
    ) {
        return false;
    }

    const bothHaveIds = leftSig.ids.length > 0 && rightSig.ids.length > 0;
    if (bothHaveIds && !hasIntersection(leftSig.ids, rightSig.ids)) {
        return false;
    }

    const bothHaveTypes = leftSig.types.length > 0 && rightSig.types.length > 0;
    if (bothHaveTypes && !hasIntersection(leftSig.types, rightSig.types)) {
        return false;
    }

    const hasNamedTokensLeft =
        leftSig.types.length > 0 ||
        leftSig.classes.length > 0 ||
        leftSig.ids.length > 0 ||
        leftSig.attrs.length > 0;
    const hasNamedTokensRight =
        rightSig.types.length > 0 ||
        rightSig.classes.length > 0 ||
        rightSig.ids.length > 0 ||
        rightSig.attrs.length > 0;
    const hasSharedNamedToken =
        hasIntersection(leftSig.types, rightSig.types) ||
        hasIntersection(leftSig.classes, rightSig.classes) ||
        hasIntersection(leftSig.ids, rightSig.ids) ||
        hasIntersection(leftSig.attrs, rightSig.attrs);
...
function isPureUniversalSelector(signature) {
    if (!signature) {
        return false;
    }
    ...
    return (
        ...
        signature.classes.length === 0 &&
        signature.ids.length === 0 &&
        signature.attrs.length === 0 &&
        signature.pseudoElements.length === 0
    );
}

function signaturesHaveNamedTokens(signature) {
    if (!signature) {
        return false;
    }
...
function signaturesShareNamedTokens(leftSig, rightSig) {
    return (
        hasIntersection(leftSig.types, rightSig.types) ||
        hasIntersection(leftSig.classes, rightSig.classes) ||
        hasIntersection(leftSig.ids, rightSig.ids) ||
        hasIntersection(leftSig.attrs, rightSig.attrs) ||
        hasIntersection(leftSig.pseudoElements, rightSig.pseudoElements)
    );
}

function signaturesHaveSpecificTokens(signature) {
    if (!signature) {
        return false;
    }
...
function signaturesShareSpecificTokens(leftSig, rightSig) {
    return (
        hasIntersection(leftSig.classes, rightSig.classes) ||
        hasIntersection(leftSig.ids, rightSig.ids) ||
        hasIntersection(leftSig.attrs, rightSig.attrs)
...
function selectorsLikelyTargetSameElement(
    leftSig,
    rightSig,
    leftTerminalSig,
    rightTerminalSig,
    ...
    const leftTarget = leftTerminalSig || leftSig;
    const rightTarget = rightTerminalSig || rightSig;

    if (!leftTarget || !rightTarget) {
        return true;
    }

    const bothHaveTargetIds =
        leftTarget.ids.length > 0 && rightTarget.ids.length > 0;
    if (
        bothHaveTargetIds &&
        !hasIntersection(leftTarget.ids, rightTarget.ids)
    ) {
        return false;
    }

    const bothHaveTargetTypes =
        leftTarget.types.length > 0 && rightTarget.types.length > 0;
    if (
        bothHaveTargetTypes &&
        !hasIntersection(leftTarget.types, rightTarget.types)
    ) {
        return false;
    }

    const leftHasPseudo = leftTarget.pseudoElements.length > 0;
    const rightHasPseudo = rightTarget.pseudoElements.length > 0;
    if (leftHasPseudo || rightHasPseudo) {
        if (!leftHasPseudo || !rightHasPseudo) {
            return false;
        }
        ...
        if (
            !hasIntersection(
                leftTarget.pseudoElements,
                rightTarget.pseudoElements,
            )
        ) {
            return false;
        }
    ...
    if (
        leftTarget.classes.length > 0 &&
        rightTarget.classes.length > 0 &&
        !hasIntersection(leftTarget.classes, rightTarget.classes)
    ) {
        return false;
    }

    if (
        leftTarget.attrs.length > 0 &&
        rightTarget.attrs.length > 0 &&
        !hasIntersection(leftTarget.attrs, rightTarget.attrs) &&
        !hasIntersection(leftTarget.classes, rightTarget.classes) &&
        !hasIntersection(leftTarget.ids, rightTarget.ids)
    ) {
        return false;
    ...
    if (
        isPureUniversalSelector(leftTarget) &&
        signaturesHaveNamedTokens(rightTarget)
    ) {
        return false;
    }

    if (
        isPureUniversalSelector(rightTarget) &&
        signaturesHaveNamedTokens(leftTarget)
    ) {
        return false;
    }

    const bothHaveNamedTargetTokens =
        signaturesHaveNamedTokens(leftTarget) &&
        signaturesHaveNamedTokens(rightTarget);

    if (
        bothHaveNamedTargetTokens &&
        !signaturesShareNamedTokens(leftTarget, rightTarget) &&
        !leftTarget.universal &&
        !rightTarget.universal
    ) {
        return false;
    }
...
function hasAtRuleAncestor(node, atRuleName, paramsKeyword = "") {
    let current = node?.parent;

    while (current) {
        if (
            current.type === "atrule" &&
            current.name === atRuleName &&
            (paramsKeyword === "" ||
                current.params.toLowerCase().includes(paramsKeyword))
        ) {
            return true;
        }
...
function classifyImportantUsage({ relativePath, selector, property, node }) {
    if (selector === "[x-cloak]" && property === "display") {
        return null;
    }

    if (
        hasAtRuleAncestor(node, "media", "prefers-reduced-motion") &&
        REDUCED_MOTION_IMPORTANT_PROPS.has(property)
    ) {
        return {
            type: "important-intentional",
            severity: "minor",
            message:
                "Intentional !important for reduced-motion safety override",
        };
    }

    if (
        hasAtRuleAncestor(node, "media", "print") &&
        PRINT_IMPORTANT_PROPS.has(property)
    ) {
        return {
            type: "important-intentional",
            severity: "minor",
            message: "Intentional !important for print-safe rendering",
        };
    }

    if (
        (relativePath === "/styles/pd.framework-bridge.css" ||
            relativePath === "/styles/pd-safe-layers.css") &&
        selector.startsWith("html[data-pd-safe]")
    ) {
        return {
            type: "important-intentional",
            severity: "minor",
            message:
                "Intentional !important for safe-mode framework bridge override",
        };
    }
...
function classifyHighSpecificityUsage({ relativePath, selector, score, spec }) {
    if (score <= 40 || spec.b > 0) {
        return null;
    }

    if (
        selector.includes(":where(") &&
        (relativePath.startsWith("/styles/pd.") ||
            relativePath === "/styles/pd-safe-layers.css")
    ) {
        return {
            type: "high-specificity-intentional",
            severity: "minor",
            message:
                "High computed specificity in :where() safe-mode selector (treated as intentional override)",
        };
    }
        ...
        root.walkRules((rule) => {
            ...
            selectors.forEach((selector) => {
                ...
                const ruleData = {
                    ...
                    score,
                    depth,
                    signature: parseSelectorSignature(selector),
                    terminalSignature: parseTerminalSelectorSignature(selector),
                    file: relativePath,
                ...
                rule.walkDecls((decl) => {
                    ...
                    if (decl.important && !relativePath.includes("utility")) {
                        const importantClassification = classifyImportantUsage({
                            relativePath,
                            selector,
                            property: decl.prop,
                            node: decl,
                        ...
                        if (!importantClassification) {
                            return;
                        }

                        issues.push({
                            type: importantClassification.type,
                            severity: importantClassification.severity,
                            file: relativePath,
                            line: decl.source?.start?.line,
                            selector,
                            property: decl.prop,
                            message: importantClassification.message,
                        });
                    }
                });
                ...
                const specificityClassification = classifyHighSpecificityUsage({
                    relativePath,
                    selector,
                    score,
                    spec,
                ...
                if (specificityClassification) {
                    issues.push({
                        type: specificityClassification.type,
                        severity: specificityClassification.severity,
                        file: relativePath,
                        line: rule.source?.start?.line,
                        selector,
                        specificity: specificityToString(spec),
                        score,
                        message: specificityClassification.message,
                    });
                }
                ...
                if (spec.b > 0) {
                    issues.push({
                        ...
                        selector,
                        message:
                            "ID selector used - prefer classes for styling",
                    });
                }
            });
        });
    ...
    allRules.forEach((rule) => {
        rule.declarations.forEach((decl) => {
            ...
            propertyMap.get(key).push({
                ...
                terminalSignature: rule.terminalSignature,
            });
        });
    });
    ...
    propertyMap.forEach((rules, property) => {
        ...
        for (let i = 0; i < rules.length; i++) {
            for (let j = i + 1; j < rules.length; j++) {
                ...
                if (
                    Math.abs(r1.score - r2.score) <= 10 &&
                    r1.value !== r2.value
                ) {
                    if (!selectorsMayOverlap(r1.signature, r2.signature)) {
                        continue;
                    }

                    if (
                        !selectorsLikelyTargetSameElement(
                            r1.signature,
                            r2.signature,
                            r1.terminalSignature,
                            r2.terminalSignature,
                        )
                    ) {
                        continue;
                    }
...
function generateMarkdownReport(
    ...
    const noFallback = allRules.filter((rule) => {
        return rule.declarations.some(
            (d) => d.value.includes("var(") && !d.value.includes(","),
        );
    });
    ...
    if (noFallback.length > 0) {
        ...
        md +=
            "These declarations use CSS variables without fallback values:\n\n";
