...
<head>
<meta charset="utf-8">
<title>Formulir Preparasi — {{ $sample?->short_description ?? 'Sampel' }}</title>
<style>
  /* DOMPDF-safe styles */
...
<body>
  ...
  <div class="meta">
    <div><strong>Sampel:</strong> {{ $sample?->short_description ?? '-' }}</div>
  ...
  <p class="muted right">Dibuat pada {{ $now->translatedFormat('l, d F Y H:i') }} WIB</p>
