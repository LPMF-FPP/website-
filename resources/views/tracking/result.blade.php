<x-app-layout>
    ...
    <x-slot name="header">
        <x-page-header
            :title="'📦 ' . __('tracking.page_title') . ' - ' . $trackingData['request_number']"
            :breadcrumbs="[[ 'label' => __('tracking.breadcrumbs.tracking'), 'href' => route('tracking.index') ], [ 'label' => __('tracking.breadcrumbs.result') ]]"
        >
            <x-slot name="actions">
                <a href="{{ route('tracking.index') }}" class="btn-sem inline-flex items-center">{{ __('tracking.actions.track_again') }}</a>
            </x-slot>
        </x-page-header>
    </x-slot>

    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6"
         x-data="trackingProgress({ initial: @js($condensed) })">
            ...
            <div class="flex flex-col gap-6">
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <h3 class="text-lg font-semibold text-sem-high">{{ __('tracking.labels.progress_testing') }}</h3>
                        <div class="flex items-center gap-3">
                            <button type="button" @click="manualRefresh()" class="text-xs px-2 py-1 rounded-md border-sem-subtle hover:bg-sem-muted transition">{{ __('tracking.actions.manual_refresh') }}</button>
                ...
                @foreach($trackingData['tracking_stages'] as $stage)
                    <li class="ml-4">
                        <div class="flex items-start gap-3">
                            <div class="mt-1 text-xl">{{ $stage['icon'] }}</div>
                            <div class="flex-1">
                                <h4 class="font-semibold text-sem-high">{{ $stage['title'] }}</h4>
                                <p class="text-sm text-sem-dim">{{ $stage['description'] }}</p>
                                @if($stage['timestamp'])
                                    <p class="text-xs mt-1 text-sem-dim">{{ $stage['timestamp'] }}</p>
                                @endif
                            </div>
                            <x-timeline-status-badge :status="$stage['status']" />
                        </div>
                    </li>
                @endforeach
            </ol>
        ...
        <div class="surface-sem shadow-sm sm:rounded-lg p-6 border-sem">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
                <div class="surface-sem-alt p-4 rounded-lg">
                    ...
                    <div class="font-semibold text-sem-high">{{ $trackingData['request_number'] }}</div>
                </div>
                <div class="surface-sem-alt p-4 rounded-lg">
                    <div class="text-2xl mb-2">👤</div>
                    <div class="text-sm text-sem-dim">{{ __('tracking.labels.investigator') }}</div>
                    <div class="font-semibold text-sem-high">{{ $trackingData['investigator']['rank'] }} {{ $trackingData['investigator']['name'] }}</div>
                </div>
                <div class="surface-sem-alt p-4 rounded-lg">
                    <div class="text-2xl mb-2">🧪</div>
                    <div class="text-sm text-sem-dim">{{ __('tracking.labels.samples_count') }}</div>
                    <div class="font-semibold text-sem-high">{{ $trackingData['samples_count'] }} sampel</div>
                </div>
                <div class="surface-sem-alt p-4 rounded-lg">
                    <div class="text-2xl mb-2">⏰</div>
                    <div class="text-sm text-sem-dim">{{ __('tracking.labels.estimated_completion') }}</div>
                    <div class="font-semibold text-sem-high">{{ date('d M Y', strtotime($trackingData['estimated_completion'])) }}</div>
                </div>
            </div>
        </div>
        ...
        <!-- Contact Info -->
        <div class="surface-sem shadow-sm sm:rounded-lg p-6 border-sem">
            <h3 class="text-lg font-semibold text-sem-high mb-4">📞 {{ __('tracking.labels.contact_info') }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="surface-sem-alt p-4 rounded-lg">
                    <h4 class="font-semibold text-primary-700 dark:text-primary-300 mb-2">{{ __('tracking.labels.investigator_submitter') }}</h4>
                    <div class="text-sm space-y-1 text-sem-high">
                        <p><strong>{{ __('tracking.misc.investigator_name') }}:</strong> {{ $trackingData['investigator']['rank'] }} {{ $trackingData['investigator']['name'] }}</p>
                        <p><strong>{{ __('tracking.misc.investigator_unit') }}:</strong> {{ $trackingData['investigator']['jurisdiction'] }}</p>
                        <p><strong>{{ __('tracking.misc.investigator_phone') }}:</strong> {{ $trackingData['investigator']['phone'] }}</p>
                    </div>
                </div>
                <div class="surface-sem-alt p-4 rounded-lg">
                    <h4 class="font-semibold text-success-700 dark:text-success-300 mb-2">{{ __('tracking.labels.lab_unit') }}</h4>
                    <div class="text-sm space-y-1 text-sem-high">
                        <p><strong>{{ __('tracking.misc.lab_address') }}:</strong> Jl. Cipinang Baru Raya No.3B 11, RT.11/RW.6, Cipinang, Kec. Pulo Gadung, Kota Jakarta Timur, Daerah Khusus Ibukota Jakarta 13240</p>
                        <p><strong>{{ __('tracking.misc.lab_phone') }}:</strong> (021) 720-0461</p>
                        <p><strong>{{ __('tracking.misc.lab_email') }}:</strong> Labmutufarmapol@gmail.com</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function trackingProgress({ initial }) {
            return {
                data: initial || { stages: [], progress_percent: 0, last_updated: null, request_number: @js($trackingData['request_number']) },
                pollInterval: null,
                pollingMs: 45000,
                ...
                lastUpdatedDisplay() { if (!this.data.last_updated) return ''; const d = new Date(this.data.last_updated.replace(' ', 'T')); return '{{ __('tracking.labels.last_updated') }}: ' + d.toLocaleString('id-ID'); },
