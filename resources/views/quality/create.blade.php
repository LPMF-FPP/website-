    ...
    <script>
        function qmhCreatePage(config) {
            return {
                ...
                canProceedStep1() {
                    if (!this.docCode || !this.title || !this.clause || !this.docType || !this.templateId) {
                        return false;
                    }
                ...
                goToPreview() {
                    ...
                    if (!this.canProceedStep1()) {
                        this.stepError = 'Lengkapi struktur dokumen terlebih dahulu sebelum lanjut ke preview.';
                        return;
                    }
                ...
                selectedTemplate() {
                    return this.templates.find((item) => Number(item.id) === Number(this.templateId)) || null;
                },
                ...
                selectedParentSopLabel() {
                    if (!this.parentSopId) {
                        return this.requiresParentSop() ? 'Belum dipilih' : 'Tidak wajib';
                    }

                    const sop = this.sopOptions.find((item) => Number(item.id) === Number(this.parentSopId));
                    return sop ? sop.label : '-';
                },

                selectedPairedIkLabel() {
                    if (!this.pairedIkId) {
                        return this.requiresPairedIk() ? 'Tanpa pasangan IK' : 'Tidak wajib';
                    }

                    const ik = this.ikOptions.find((item) => Number(item.id) === Number(this.pairedIkId));
                    return ik ? ik.label : '-';
                },
