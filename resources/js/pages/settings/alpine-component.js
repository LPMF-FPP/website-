    ...
    Alpine.data("settingsPageAlpine", () => {
        ...
        return {
            ...
            set activeSection(value) {
                ...
                if (value === "templates") {
                    if (
                        !this.documentTemplatesLoading &&
                        this.documentTemplateState.templates.length === 0
                    ) {
                        this.loadDocumentTemplates();
                    }
                    // Refresh template editor when section becomes visible
                    if (
                        previousSection !== "templates" &&
                        this.templateEditorInstance
                    ) {
                        this.$nextTick(() => {
            ...
            getTemplatesByType(type) {
                return (this.client.state.templates || []).filter(
                    (tpl) => tpl.type === type,
                );
            },
            timezones:
                Array.isArray(optionValues.timezones) &&
                optionValues.timezones.length
                    ? optionValues.timezones
                    : ["Asia/Jakarta", "Asia/Makassar", "Asia/Jayapura", "UTC"],
            dateFormats:
                Array.isArray(optionValues.date_formats) &&
                optionValues.date_formats.length
                    ? optionValues.date_formats
                    : ["DD/MM/YYYY", "YYYY-MM-DD", "DD-MM-YYYY"],
            numberFormats: (Array.isArray(optionValues.number_formats) &&
            optionValues.number_formats.length
                ? optionValues.number_formats
                : ["1.234,56", "1,234.56"]
            ).map((fmt) => ({ value: fmt, label: fmt })),
            languages: (Array.isArray(optionValues.languages) &&
            optionValues.languages.length
            ...
            storageDrivers:
                Array.isArray(optionValues.storage_drivers) &&
                optionValues.storage_drivers.length
                    ? optionValues.storage_drivers
                    : ["public"],
            storagePathInfo:
                optionValues.storage_path_info ||
                "storage/app/public/investigators/{investigator}/{request}/",
            ...
            init() {
                ...
                this.client.state.form = this.client.mergeDefaults(
            ...
            ensureLocaleDefaults() {
                ...
                if (
                    ...
                    typeof this.client.state.form.locale !== "object"
                ) {
                    this.client.state.form.locale = {};
                }
                ...
                if (
                    !driver ||
                    (allowedDrivers.size && !allowedDrivers.has(driver))
                ) {
            ...
            ensureIkuDefaults() {
                ...
                if (
                    ...
                    typeof this.client.state.form.iku !== "object"
                ) {
                    this.client.state.form.iku = {};
                }
                ...
                if (
                    ...
                    typeof this.client.state.form.iku.target_samples_by_year !==
                        "object" ||
                    Array.isArray(
                        this.client.state.form.iku.target_samples_by_year,
                    )
                ...
                this.client.state.form.iku.sources.A ??=
                    "requests_completed_count";
                this.client.state.form.iku.sources.B ??=
                    "requests_submitted_count";
                this.client.state.form.iku.sources.C ??=
                    "samples_completed_count";
            ...
            getIkuWeightSum() {
                ...
                return (
                    (parseInt(weights.registration) || 0) +
                ...
                try {
                    ...
                    if (response.ok) {
                        this.client.state.sectionStatus.iku = {
                            message:
                                response.message ||
                                "Pengaturan IKU berhasil disimpan.",
                    ...
                    this.client.state.sectionErrors.iku =
                        error.message || "Gagal menyimpan pengaturan IKU.";
                } finally {
            ...
            async refreshIkuPreview() {
                ...
                try {
                    const response = await this.client.apiFetch(
                        this.client.api.ikuPreview,
                    );
                    if (response.ok && response.iku) {
                        this.ikuPreview.data = response.iku;
                    } else {
                        throw new Error(
                            response.error || "Gagal memuat preview IKU.",
                        );
                    }
                } catch (error) {
                    console.error("IKU preview error:", error);
                    this.ikuPreview.error =
                        error.message || "Gagal memuat preview IKU.";
            ...
            downloadSurveyExport() {
                if (
                    !this.surveyExport.startDate ||
                    !this.surveyExport.endDate
                ) {
            ...
            promptActivate(type) {
                if (!this.client.state.templates.length) {
                    this.client.setSectionError(
                        "templates",
                        "Belum ada template diunggah.",
                    );
                    return;
                }
                const code = prompt(
            ...
            previewActiveTemplate(type) {
                ...
                if (!tpl) {
                    this.client.setSectionError(
                        "templates",
                        "Tidak ada template aktif untuk tipe ini.",
                    );
                    return;
                }
                let targetId = tpl.id;
                if (!targetId && tpl.code) {
                    const match = this.client.state.templates.find(
                ...
                if (!targetId) {
                    this.client.setSectionError(
                        "templates",
                        "Template aktif belum memiliki file.",
                    );
                    return;
                }
            ...
            documentTemplateState: {
                templates: [],
                templatesByType: {},
                documentTypes: normalizeDocumentTypes(
                    optionValues.document_types,
                ),
            ...
            normalizeTemplateResponse(data) {
                ...
                if (
                    activeFromResponse &&
                    typeof activeFromResponse === "object"
                ) {
                ...
                const documentTypes = normalizeDocumentTypes(
                    data?.documentTypes ||
                        data?.document_types ||
                        data?.types ||
                        this.documentTemplateState.documentTypes,
                );
            ...
            async loadDocumentTemplates(options = {}) {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(
                            `Gagal memuat template (${response.status})`,
                        );
                    }
                    ...
                    this.documentTemplateState.templatesByType =
                        normalized.templatesByType;
                    this.documentTemplateState.documentTypes =
                    ...
                    this.documentTemplateError =
                        error.message ||
                        "Gagal memuat daftar template. Silakan coba lagi.";
                } finally {
            ...
            async syncTemplateSelection(options = {}) {
                ...
                if (fallbackType) {
                    ...
                    const templatesForType = this.getTemplatesForEditor(
                        fallbackType,
                        { ignoreFilters: true },
                    );
                    const activeId =
                        this.documentTemplateState.activeTemplateByType?.[key];
                    const activeTemplate = activeId
                        ? templatesForType.find(
                              (tpl) => String(tpl.id) === String(activeId),
                          )
                        : null;
                    ...
                    if (
                        targetId &&
                        !this.findTemplateById(targetId, fallbackType)
                    ) {
                        targetId = "";
                    }
                    ...
                    if (!targetId && templatesForType.length) {
                        const first =
                            templatesForType.find((tpl) => !tpl.is_draft) ||
                            templatesForType[0];
                        targetId = first?.id ?? "";
                    }
            ...
            async uploadDocumentTemplate() {
                if (
                    !this.documentTemplateForm.type ||
                    !this.documentTemplateForm.name ||
                    !this.documentTemplateForm.file
                ) {
                    this.documentTemplateError =
                ...
                try {
                    ...
                    const response = await fetch(
                        "/api/settings/document-templates/upload",
                        {
                            method: "POST",
                            headers: {
                                Accept: "application/json",
            ...
            getTemplatesForEditor(type = null, options = {}) {
                ...
                const templates =
                    this.documentTemplateState.templatesByType[key] || [];
            ...
            findTemplateById(templateId, type = null) {
                ...
                return (
                    list.find((tpl) => String(tpl.id) === String(templateId)) ||
                    null
                );
            },

            getDocumentTypeMeta(type) {
                return (this.documentTemplateState.documentTypes || []).find(
                    (option) => option.code === type || option.value === type,
                );
            },

            templateEditorFormats() {
                const meta = this.getDocumentTypeMeta(
                    this.documentTemplateState.selectedDocumentType,
                );
            ...
            formatTemplateOption(tpl) {
                const activeId =
                    this.documentTemplateState.activeTemplateByType?.[
                        (tpl.type || "").toLowerCase()
                    ];
                const badge = tpl.is_draft
                    ? "Draft"
                    : tpl.id && String(tpl.id) === String(activeId)
                      ? "Aktif"
                      : "";
                const versionLabel = tpl.version
                    ? `v${tpl.version}`
                    : tpl.is_draft
                      ? "Draft"
                      : "";
                const suffix = [versionLabel, badge]
                    .filter(Boolean)
                    .join(" • ");
            ...
            async onModalTemplateChange() {
                ...
                if (tpl) {
                    ...
                    this.templateEditorModal.name =
                        tpl.name || this.templateEditorModal.name;
                    this.templateEditorModal.format =
                        tpl.format || this.templateEditorModal.format;
                    this.templateEditorModal.engine =
                        tpl.render_engine || this.templateEditorModal.engine;
            ...
            removeDraftsForType(type) {
                ...
                const list =
                    this.documentTemplateState.templatesByType[key] || [];
                const draftIds = list
                    .filter((tpl) => tpl.is_draft)
                    .map((tpl) => String(tpl.id));

                this.documentTemplateState.templatesByType[key] = list.filter(
                    (tpl) => !tpl.is_draft,
                );
                if (draftIds.length) {
                    this.documentTemplateState.templates =
                        this.documentTemplateState.templates.filter(
            ...
            async onDocumentTypeChange(options = {}) {
                ...
                const templatesForType = this.getTemplatesForEditor(type, {
                    ignoreFilters: true,
                });
                const activeId =
                    this.documentTemplateState.activeTemplateByType?.[key];
                ...
                if (activeTemplate?.render_engine) {
                    this.documentTemplateEditor.renderEngine =
                        activeTemplate.render_engine;
                }
            ...
            async openTemplateEditorModal(tpl) {
                ...
                this.templateEditorModal.meta = {
                    ...
                    version: tpl.version || "",
                    status:
                        tpl.status || (tpl.is_active ? "active" : "inactive"),
                    is_active: !!tpl.is_active,
                };
            ...
            async ensureTemplateEditorInModal() {
                ...
                if (this.templateEditorInstance) {
                    const {
                        ...
                        waitForEditorLoad,
                    } = await import("./template-editor.js");
                    if (!isAlive(this.templateEditorInstance)) {
                ...
                if (!container) {
                    this.templateEditorModal.error =
                        "Container editor tidak ditemukan.";
                    console.error(
                        "Template editor container not found in ensureTemplateEditorInModal",
                    );
                    return null;
                }
                ...
                if (container.offsetParent === null) {
                    this.templateEditorModal.error =
                        "Container editor belum visible.";
                    console.error(
                        "Template editor container still hidden in ensureTemplateEditorInModal",
                    );
                    return null;
                }
                ...
                try {
                    const { createTemplateEditor, waitForEditorLoad, isAlive } =
                        await import("./template-editor.js");
                    this.templateEditorInstance = await createTemplateEditor({
                        ...
                        options: {
                            height: "100%",
                            blocksContainer:
                                this.$refs.templateEditorModalBlocks,
                            stylesContainer:
                                this.$refs.templateEditorModalStyles,
                            traitsContainer:
                                this.$refs.templateEditorModalTraits,
                            layersContainer:
                                this.$refs.templateEditorModalLayers,
                            onChange: () => {
            ...
            async loadTemplate(templateId) {
                ...
                const { isAlive, refreshTemplateEditor, waitForEditorLoad } =
                    await import("./template-editor.js");
                await waitForEditorLoad("modal");
                if (!isAlive(editor)) {
                ...
                this.templateEditorModal.name =
                    detail.name || this.templateEditorModal.name;
                this.templateEditorModal.format =
                    detail.format || this.templateEditorModal.format;
                this.templateEditorModal.engine =
                    detail.render_engine || this.templateEditorModal.engine;
                this.templateEditorModal.id =
                    detail.id ?? this.templateEditorModal.id;
                this.templateEditorModal.documentType =
                    detail.type || this.templateEditorModal.documentType;
                this.templateEditorModal.meta = {
                    doc_type:
                        detail.type ||
                        this.templateEditorModal.documentType ||
            ...
            revokeModalPreviewUrl() {
                if (this.templateEditorModal.previewObjectUrl) {
                    URL.revokeObjectURL(
                        this.templateEditorModal.previewObjectUrl,
                    );
            ...
            async saveTemplateFromEditor() {
                ...
                if (!payload.type || !payload.name) {
                    this.templateEditorModal.error =
                        "Lengkapi tipe dan nama template.";
                    return;
                }
                ...
                try {
                    ...
                    this.templateEditorModal.name =
                        template.name || this.templateEditorModal.name;
                    await this.loadDocumentTemplates({
                        preserveSelection: true,
            ...
            async previewFromModal(format) {
                ...
                if (!typeSlug) {
                    ...
                    console.error(
                        "❌ Unknown document type for preview:",
                        type,
                    );
                    this.templateEditorModal.previewLoading = false;
                    return;
                ...
                try {
                    if (format === "html") {
                        ...
                        if (!response.ok) {
                            throw new Error(
                                `Gagal memuat preview (${response.status})`,
                            );
                        }
                    ...
                    this.templateEditorModal.error =
                        error.message || "Gagal memuat preview.";
                } finally {
                    this.templateEditorModal.previewLoading = false;
                }
            },
            ...
            async refreshTemplatePreview() {
                ...
                try {
                    ...
                    this.documentTemplateEditor.error =
                        error.message || "Gagal memuat preview template.";
                } finally {
                    this.previewLoading = false;
                }
            },

            async refreshTemplatePreviewFor({
                type,
                format,
                templateId = null,
            } = {}) {
            ...
            async ensureTemplateEditor() {
                ...
                if (this.activeSection !== "templates") {
                    const errorMsg =
                        "Section Template Dokumen belum aktif. Aktifkan section terlebih dahulu.";
                ...
                if (!container) {
                    const errorMsg =
                        "Container element tidak ditemukan. Pastikan section Template Dokumen aktif.";
                    console.error("❌", errorMsg, {
                ...
                if (container.offsetParent === null) {
                    ...
                    if (container.offsetParent === null) {
                        const errorMsg =
                            "Container harus visible sebelum init editor.";
                        console.error(
                            "❌",
                            errorMsg,
                            "offsetParent:",
                ...
                this.templateEditorInitPromise = import("./template-editor.js")
                    .then(({ createTemplateEditor }) => {
                        ...
                        return createTemplateEditor({
                            ...
                            options: {
                                blocksContainer:
                                    this.$refs.documentTemplateBlocks,
                                stylesContainer:
                                    this.$refs.documentTemplateStyles,
                                traitsContainer:
                                    this.$refs.documentTemplateTraits,
                                layersContainer:
                                    this.$refs.documentTemplateLayers,
                                onChange: () => {
                                    this.templateEditorDirty = true;
                                },
                            },
            ...
            refreshTemplateEditor() {
                ...
                import("./template-editor.js")
                    ...
                    .catch((error) => {
                        console.warn(
                            "Failed to refresh template editor:",
                            error,
                        );
                    });
            },
            ...
            async createNewTemplate(options = {}) {
                ...
                const format =
                    meta?.defaultFormat ??
                    this.documentTemplateEditor.format ??
                    "pdf";
                const renderEngine =
                    this.documentTemplateEditor.renderEngine || "dompdf";
                ...
                if (addToList) {
                    ...
                    this.documentTemplateState.templatesByType[key] = [
                        draft,
                        ...existing.filter((tpl) => !tpl.is_draft),
                    ];
            ...
            async startNewEditorTemplate(draft = null) {
                ...
                try {
                    ...
                    if (!editor) {
                        console.error(
                            "❌ Failed to ensure editor for new template",
                        );
                        return;
                    }
                    ...
                    this.documentTemplateEditor.name =
                        draft?.name || `${typeLabel} Draft`;
                    this.documentTemplateEditor.renderEngine =
                        draft?.render_engine ||
                        this.documentTemplateEditor.renderEngine ||
                        "dompdf";
                    this.documentTemplateEditor.format =
                        draft?.format || meta?.defaultFormat || "pdf";
                    this.documentTemplateEditor.previewUrls = {
                        html: "",
                        pdf: "",
                    };
                    this.documentTemplateEditor.error = "";
            ...
            async loadTemplateDetail(templateId) {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(
                            error.message || "Gagal memuat detail template",
                        );
                    }
            ...
            async saveEditorTemplate() {
                ...
                try {
                    ...
                    this.documentTemplateEditor.success =
                        data.message || "Template tersimpan";
                    this.templateEditorDirty = false;
            ...
            async activateEditorTemplate() {
                ...
                if (!templateId) {
                    this.documentTemplateEditor.error =
                        "Tidak ada template yang dipilih.";
                    return;
                }
                await this.activateDocumentTemplate(templateId);
                await this.loadDocumentTemplates({
            ...
            async activateDocumentTemplate(templateId) {
                if (
                    !confirm(
                        "Activate this template? It will replace the currently active template.",
                    )
                ) {
                    return;
                }
                ...
                try {
                    const response = await fetch(
                        `/api/settings/document-templates/${templateId}/activate`,
                        {
                            method: "PUT",
                            headers: {
                                Accept: "application/json",
            ...
            destroyTemplateEditor() {
                ...
                import("./template-editor.js")
                    ...
                    .catch((error) => {
                        console.warn(
                            "Error destroying template editor:",
                            error,
                        );
                    })
                    .finally(() => {
            ...
            toggleWorkDay(dayNumber) {
                const workDays =
                    this.client?.state?.form?.monitoring_logging?.environment
                        ?.work_days || [];
                const index = workDays.indexOf(dayNumber);
                if (index > -1) {
                ...
                if (this.client?.state?.form?.monitoring_logging?.environment) {
                    this.client.state.form.monitoring_logging.environment.work_days =
                        [...workDays];
                }
            },

            async saveMonitoringLoggingSettings() {
                this.client.state.loadingSections.monitoring_logging = true;
                ...
                try {
                    const payload = {
                        monitoring_logging: this.client.toPlainObject(
                            this.client.state.form.monitoring_logging,
                        ),
                    };

                    const response = await this.client.apiFetch(
                        this.client.api.monitoringLogging,
                        {
                            method: "PUT",
                            body: payload,
                        },
                    ...
                    if (response.ok) {
                        this.client.state.sectionStatus.monitoring_logging = {
                            message:
                                response.message ||
                                "Pengaturan Monitoring & Logging berhasil disimpan.",
                            intentClass: "text-green-600",
                        };
                    } else {
                        throw new Error(
                            response.error || "Gagal menyimpan pengaturan.",
                        );
                    }
                } catch (error) {
                    console.error("Monitoring/Logging save error:", error);
                    this.client.state.sectionErrors.monitoring_logging =
