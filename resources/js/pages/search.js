...
export function safeUrl(url, baseOrigin) {
    ...
    try {
        ...
        const disallowedProtocols = new Set([
            "javascript",
            "data",
            "mailto",
            "tel",
    ...
    const normalized = {
        ...
        type:
            typeof state.type === "string"
                ? state.type.trim()
                : String(state.type || "").trim(),
        sort: SORT_OPTIONS.includes(state.sort) ? state.sort : DEFAULT_SORT,
        page: Math.max(1, Number.parseInt(state.page, 10) || 1),
        per_page: Math.max(
            1,
            Number.parseInt(state.per_page, 10) || defaultPerPage,
        ),
        filters: {},
    };
    const filters =
        state.filters && typeof state.filters === "object" ? state.filters : {};
    ...
    return normalizeState(
        {
            ...
            per_page:
                Number.parseInt(params.get("per_page"), 10) || defaultPerPage,
            filters: filterObj,
        },
        defaultPerPage,
    );
...
export function isSearchReady(state = {}) {
    ...
    const hasType =
        typeof state.type === "string" && state.type.trim().length > 0;
    ...
    return normalizeState(
        {
            ...
            per_page: els.perPage
                ? Number.parseInt(els.perPage.value, 10) || defaultPerPage
                : defaultPerPage,
            filters,
        },
        defaultPerPage,
...
export function applyStateToForm(state, els) {
    ...
    if (els.q) {
        els.q.value = normalized.q;
    }
    if (els.type) {
        els.type.value = normalized.type;
    }
    if (els.sort) {
        els.sort.value = normalized.sort;
    }
    if (els.status) {
        els.status.value = normalized.filters.status || "";
    }
    if (els.dateFrom) {
        els.dateFrom.value = normalized.filters.date_from || "";
    }
    if (els.dateTo) {
        els.dateTo.value = normalized.filters.date_to || "";
    }
    if (els.facilityId) {
        els.facilityId.value = normalized.filters.facility_id || "";
    }
    if (els.perPage) {
        els.perPage.value = String(normalized.per_page);
    }
}
...
export function renderResultsHTML(items = []) {
    ...
    return items
        .map((item) => {
            ...
            const metaParts = [];
            if (item?.type) {
                metaParts.push(`<span>${escapeHTML(item.type)}</span>`);
            }
            if (item?.created_at) {
                metaParts.push(
...
function renderSkeleton(count = 3) {
    return Array.from({ length: count })
        .map(
            () =>
                '<div class="result-card skeleton" style="height: 96px;"></div>',
        )
        .join("");
}
...
export function initSearchPage(rootEl) {
    ...
    const defaultPerPage =
        Number.parseInt(rootEl.dataset.defaultPerPage, 10) || 15;
    if (!apiEndpoint) {
        return;
    }
    ...
    let state = normalizeState(
        parseStateFromUrl(window.location.search, defaultPerPage),
        defaultPerPage,
    );
    let controller = null;
    let lastRequestId = 0;
    ...
    async function fetchResults() {
        if (!isSearchReady(state)) {
            ...
            showError(null);
            updateStatus(
                "Siap mencari. Masukkan kata kunci minimal 2 karakter atau gunakan filter.",
            );
            updatePagination({
                page: state.page,
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 422) {
                    const validationMessages = errorPayload?.errors
                        ...
                        : "";
                    const message =
                        errorPayload?.message || "Permintaan tidak valid.";
                    throw new Error(
                        `${message}${validationMessages ? ` — ${validationMessages}` : ""}`,
                    );
                }
                const generic = errorPayload?.message
            ...
            const perPage =
                Number(pagination.per_page) || state.per_page || defaultPerPage;
            ...
            if (items.length === 0) {
                ...
                updateStatus(
                    "Tidak ada hasil yang cocok dengan pencarian ini.",
                );
            } else {
                ...
                updateStatus(
                    `Menampilkan ${start}-${end} dari ${total} hasil.`,
                );
            }
            updatePagination({
                page: state.page,
    ...
    els.reset?.addEventListener("click", () => {
        state = normalizeState(
            { per_page: defaultPerPage, page: 1 },
            defaultPerPage,
        );
        applyStateToForm(state, els);
        syncUrl(state, { pushHistory: true }, defaultPerPage);
