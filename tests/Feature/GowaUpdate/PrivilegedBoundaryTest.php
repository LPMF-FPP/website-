...
it('keeps the submit helper disabled until both gates are enabled', function (): void {
...
it('creates one deterministic pending request before starting the unit', function (): void {
    ...
    $command = sprintf(
        'PATH=%s:/usr/bin:/bin GOWA_UPDATER_ENABLED=1 GOWA_UPDATER_NO_SOCKET_GATE=1 GOWA_UPDATER_CAPABILITY_MANIFEST=%s GOWA_UPDATER_REQUEST_ROOT=%s bash %s 00000000-0000-4000-8000-000000000000',
        escapeshellarg($bin), escapeshellarg($manifest), escapeshellarg($root.'/requests'), escapeshellarg(base_path('ops/gowa-updater/lpmf-gowa-submit'))
    );
